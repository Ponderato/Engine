...
FrustumStep::FrustumStep(Camera* camera, Program& program) : camera(camera), program(program){}

void FrustumStep::RenderStep() {

	DrawCamerasFrustum();
}

void FrustumStep::DrawCamerasFrustum() {
	...
	for (int i = 0; i < inactiveCameras.size(); i++) {
		...
		if (inactiveCameras.at(i)->GetDrawFrustum()) {
			...
			program.SetVec3("color", inactiveCameras.at(i)->GetFrustumColor());
